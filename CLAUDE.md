...
- **只能輸入數字**：元件會擋掉文字、`e`、`+`、`-` 等字元，貼上時自動去掉千分位、文字並把全形數字轉半形。預設只收 0 以上的整數，依欄位性質開選項：
  - `allowDecimal`：尺寸（cm／mm）、工時、百分比、採購數量／單價等可有小數的欄位。台幣金額維持整數。
  - `allowNegative`：價差、盤點調整、薪資／對帳「調整」、發票折扣列等本來就可為負的欄位。
  - `keepZero`：「空白」和「0」意義不同時開啟（空白＝沿用全域／未設定／未盤點／自動推算，0＝明確設為 0），值為 0 時照常顯示 0。
...
- 清空欄位時 `onValueChange` 會收到 `null`，由呼叫端決定存成什麼：
  ...
  - 表單 state 存字串的欄位：`value` 直接傳字串，`onValueChange={(v) => setX(toNumericText(v))}`（`toNumericText` 在 `@/lib/numeric-input`，清空 → `""`）。
- 統一編號、銀行帳號、發票號碼這類「代碼」可能有開頭 0，**不是數字**，維持 `type="text"` ＋ `inputMode="numeric"`，不要改用 `NumericInput`。
