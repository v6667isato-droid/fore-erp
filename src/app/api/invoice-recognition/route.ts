...
- format_code（進項憑證格式代號）依票面型式判斷：
  ...
  「22」＝傳統收銀機長條發票（二聯式、內含稅）：長條型紙卷、常為粉紅／黃等有色紙、
  　　　　整張機器列印、沒有 QR code——不論票面有沒有印買方統編，一律判 22；
  收銀機發票的買方統編常印成「統編:12345678」「買:12345678」等字樣（多在票面上緣、日期附近），
  有找到請照樣填入 buyer_tax_id（但不影響 format_code 判定）；
  無法判斷時輸出 null
...
const CLAUDE_TAX_OUTPUT_SCHEMA = {
  ...
  properties: {
    ...
    format_code: {
      type: ["string", "null"],
      description:
        "格式代號：25＝電子發票證明聯、21＝手開／手寫發票、22＝傳統收銀機長條發票（不論有無買方統編）；無法判斷 null",
    },
    field_boxes: {
      type: ["object", "null"],
