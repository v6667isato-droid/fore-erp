...
/** 欄位級小框的檢查：同 sanitizeCropBox，但欄位常是窄長條（高度可能只有 10–30），最小尺寸放寬 */
export function sanitizeFieldBox(raw: unknown): InvoiceCropBox | null {
  if (!raw || typeof raw !== "object") return null;
  const r = raw as Record<string, unknown>;
  const nums = [r.x0, r.y0, r.x1, r.y1].map((v) =>
    typeof v === "number" && Number.isFinite(v) ? Math.min(1000, Math.max(0, v)) : null,
...
export interface InvoiceImageViewerProps {
  ...
  /** 目前 focus／hover 的單一欄位範圍：有值時紅框只畫這個（取代 fieldsBox 大框） */
  highlightBox?: InvoiceCropBox | null;
}

/** 發票照片檢視：AI 裁切自動聚焦＋滾輪縮放＋拖曳平移，雙擊回復 */
export function InvoiceImageViewer({ src, alt, cropBox, fieldsBox = null, highlightBox = null }: InvoiceImageViewerProps) {
  ...
  // 換張圖時重置（onLoad 會再套用聚焦）；在 render 中偵測 key 變化重置，避免 effect 裡連鎖 setState
  const cropKey = cropBox ? `${cropBox.x0},${cropBox.y0},${cropBox.x1},${cropBox.y1}` : "";
  const resetKey = `${src}|${cropKey}`;
  ...
  // 紅框：focus 中的單一欄位＞欄位區（fields_box）＞整張紙；換算成 object-contain 後的像素位置，跟著圖片一起縮放平移
  const redBox = highlightBox ?? fieldsBox ?? cropBox;
