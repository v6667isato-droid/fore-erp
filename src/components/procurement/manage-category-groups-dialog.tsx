...
import type { CategoryGroup, CategoryGroupTable } from "@/lib/category-groups";

export interface ManageCategoryGroupsDialogProps {
  /** 主類別群組資料表 */
  table: CategoryGroupTable;
  /** 存副類別文字的主檔表（改名時會一併更新） */
  subjectTable: "vendors" | "procurement_materials";
  /** 主檔的類別欄位名 */
  subjectColumn: "main_category" | "item_category";
  /** 主檔名稱，用於說明文字，例：「廠商」「物料」 */
  subjectLabel: string;
  /** dialog 標題，例：「管理廠商主類別」 */
  title: string;
  /** 主類別範例名稱，用於說明與 placeholder */
  exampleName: string;
  /** 既有副類別（主檔類別欄位彙整） */
  categories: string[];
...
/** 主類別管理：新增／改名／刪除主類別，並設定既有副類別的歸屬與改名 */
export function ManageCategoryGroupsDialog({
  ...
  groups,
  onSuccess,
}: ManageCategoryGroupsDialogProps) {
  ...
  const descId = `manage-${table}-desc`;
  ...
  async function onSave() {
    ...
    setSaving(true);
    // 先套用副類別改名：把用到舊名的資料全部改成新名（改成既有名稱＝合併類別）
    const changed = sortedCategories.filter((c) => finalName(c) !== c);
    for (const oldName of changed) {
      const { error } = await supabase
        .from(subjectTable)
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- 欄位名由 props 決定
        .update({ [subjectColumn]: finalName(oldName) } as any)
        .eq(subjectColumn, oldName);
      if (error) {
        setSaving(false);
    ...
    if (toUpdate.length > 0) {
      const { error } = await supabase.from(table).upsert(toUpdate, { onConflict: "id" });
      if (error) {
        setSaving(false);
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p id={descId} className="mt-1 text-sm text-muted-foreground">
                建立主類別（例：{exampleName}），再把既有類別（副類別）歸到主類別底下
              </p>
            </div>
          ...
          <div className="mt-4 space-y-5">
            <section className="space-y-2">
                ...
                {draftGroups.map((g) => (
                  <div key={g.key} className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      title={`刪除主類別（底下副類別會變回未分類，不影響${subjectLabel}資料）`}
                    >
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  placeholder={`新主類別名稱，例：${exampleName}`}
                  aria-label="新主類別名稱"
                />
            ...
            <section className="space-y-2">
              ...
              {sortedCategories.length === 0 ? (
                <p className="text-sm text-muted-foreground">尚無{subjectLabel}類別（先在{subjectLabel}資料填類別）</p>
              ) : (
                  ...
                  {sortedCategories.map((c) => (
                    <div key={c} className="flex items-center justify-between gap-3 px-3 py-2">
                      <input
                        ...
                        title={`可直接改名；存檔時使用此類別的${subjectLabel}會一併更新`}
                      />
                      <select
                        value={assignment[c] ?? ""}
