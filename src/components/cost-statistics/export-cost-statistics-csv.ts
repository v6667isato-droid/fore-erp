...
export function exportCostStatisticsCsv(args: {
  ...
  const summaryRows = [
    ...
    csvRow(["木料含攤提", Math.round(current.totalPurchaseWood)]),
    csvRow(["其他攤提", Math.round(current.totalPurchaseAmortized)]),
  ...
  for (const { year, snapshot } of snapshots) {
    snapshotSections.push([
      ...
      csvRow(["木料含攤提", Math.round(snapshot.totalPurchaseWood)]),
      csvRow(["其他攤提", Math.round(snapshot.totalPurchaseAmortized ?? 0)]),
